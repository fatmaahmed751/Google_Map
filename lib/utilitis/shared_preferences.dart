...
class SharedPref{
  ...
  static Future<void> setSecondaryColor({required Color color}) async {
    await prefs.setInt(_secondaryColor, color.toARGB32());
  }
  ...
  static String getCurrentThemeMode() {
    ...
    if (themeJson == null) return  "light";// default fallback

    final theme = ThemeModel.fromJson(json.decode(themeJson));
    return theme.isDark ? "dark" : "light";
  }

  static bool isDarkMode() {
    final themeJson = prefs.getString(_themeKey);
    if (themeJson == null) return false; // ✅ default is light mode
    final theme = ThemeModel.fromJson(json.decode(themeJson));
    return theme.isDark;
  }
